...
import { bookUserUpcomingAppointment } from '@/utils/api-handler/patient/page'
    ...
    useEffect(() => {
        if (session?.user) {
            (async () => {
                setLoader(true)
                try {
                    let data = await bookUserUpcomingAppointment({ user_id: session?.user?.id });
                    if (data && data?.BookUserUpcomingAppointment) {
                        setLoader(false);
                        setUpcoming(data?.BookUserUpcomingAppointment);
                    } else {
                        setLoader(false);
                        setUpcoming([]);
                    }
