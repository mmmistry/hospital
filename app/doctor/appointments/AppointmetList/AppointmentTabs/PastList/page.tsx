...
import { getPastAppoinmentList } from '@/utils/api-handler/doctor/page'
...
const PastList: FC = (): ReactElement => {
    ...
    if (session?.user) {
        (async () => {
            setLoader(true)
            try {
                let data = await getPastAppoinmentList({ doctor_id: session?.user?.id });
                if (data && data?.BookPastAppointment) {
                    setLoader(false);
                    setPastList(data?.BookPastAppointment);
                } else {
                    setLoader(false);
                    setPastList([]);
                }
                                        ...
                                        return (
                                            <tr className='bg-transparent transition-all hover:bg-secondaryLight' key={`list-${key}`}>
                                                <td className='text-sm px-4 py-3 text-left'>
                                                    <Nextlink href={'/doctor/appointment-details/' + 774}>{GetUserDetails[0]?.full_name}</Nextlink>
                                                </td>
